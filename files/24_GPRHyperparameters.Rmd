...
- GPR require the correlation function $C(x,x')$ to quantify how quickly the relationship between points decays.
- The lengthscale of decay can also be tuned by adding a hyperparameter $\theta$:
\[ C(x,x') = \exp\left\{-\frac{\lVert x-x' \rVert^2}{\theta} \right\}. \]
```{r}
library(latex2exp)
xs <- seq(-3,3,by=0.01)
...
text(1,0.4,TeX("$\\theta =1$"),col="red")
text(-1.5,0.4,TeX("$\\theta =2$"),col="blue")
```
...
\pause
- Since the nugget $g$ and the lengthscale $\theta$ alter $C_n$, all three of the parameters must be estimated simultaneously by optimization.
- Note that this optimization requires inverting $\Sigma_n$ at each iteration. **Tuning a GPR model can be more expensive than training it!**
...
- `d`=$\theta$, which we initially set to 1.
...
- `param=c("d","g")` asks `mleGP` to tune both the lengthscale and nugget. The scale $\tau^2$ is tuned automatically.
- `drange` and `grange` are vectors of bounds for $\theta$ and $g$.
  - We want a range large enough to avoid hitting the bounds, but small enough to make the search efficient.
  - $\theta=2$ is relatively large for our problem since $-3 \le x \le 3$.
...
- The lengthscale $\theta$ may not be. In particular, $\theta$ could be different for each dimension.
- Imagine optimizing reaction yield based on time, temperature, and substrate. Small changes in temperature may have big effects (small $\theta$), while the yield may be insensitive to changes in time (large $\theta$).
- Dimensions with longer lengthscales require fewer data for prediction.

\pause
- An *isotropic* model assumes parameters are fixed over all dimensions. An *anisotropic* model assumes parameters like $\theta$ vary by dimension. Anisotropic models are also called *separable*.
...
\[ C(x,x') = \exp\left\{- \sum_{k=1}^m\frac{(x_k-x_k')^2}{\theta_k} \right\}. \]
...
- For best performance, GPR models must be tuned to find a scale $\tau^2$, nugget $g$, and lengthscale $\theta$ that matches the training data.
